...
//        form.setCreateDate(new Date());
        ...
        arep.saveItem(new Date(), form.getDescription(), new Date(), form.getLinkTo(), form.getTheme(), form.getHeader());
    model.addAttribute("createdate", form.getLastChange());
    return "enter";
    }
     @GetMapping("/find")
...
//    model.addAttribute("article1", article1);
    ...
    public String saveArticle(@RequestBody String response, Model model, @ModelAttribute("article")Article article){
        System.out.println("response "+response);
        JSONObject obj = new JSONObject(response);
         arep.updateDescription(Integer.parseInt(obj.getString("id1")), obj.getString("text"), new Date());
         model.addAttribute("article", article);
       return "newhtml";
    }
